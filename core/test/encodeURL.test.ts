...
describe('encodeURL', () => {
    describe('TransferRequestURL', () => {
        it('encodes a URL', () => {
            ...
            const trzToken = Keypair.generate().publicKey;
            ...
            const url = encodeURL({ recipient, amount, trzToken, reference, label, message, memo });

            expect(String(url)).toBe(
                `trezoa:${recipient}?amount=0.000000001&tpl-token=${trzToken}&reference=${reference1}&reference=${reference2}&label=${label}&message=${message}&memo=${memo}`
            );
        });
        ...
        it('encodes a url with recipient', () => {
            ...
            expect(String(url)).toBe(`trezoa:${recipient}`);
        });
        ...
        it('encodes a url with recipient and amount', () => {
            ...
            expect(String(url)).toBe(`trezoa:${recipient}?amount=1`);
        });
        ...
        it('encodes a url with recipient, amount and token', () => {
            ...
            const url = encodeURL({ recipient, amount, trzToken });

            expect(String(url)).toBe(`trezoa:${recipient}?amount=1.01&tpl-token=${trzToken}`);
        });
        ...
        it('encodes a url with recipient, amount and references', () => {
            ...
            expect(String(url)).toBe(`trezoa:${recipient}?amount=100000.123456&reference=${reference1}`);
        });
        ...
        it('encodes a url with recipient, amount and label', () => {
            ...
            expect(String(url)).toBe(`trezoa:${recipient}?amount=1.99&label=${label}`);
        });
        ...
        it('encodes a url with recipient, amount and message', () => {
            ...
            expect(String(url)).toBe(`trezoa:${recipient}?amount=1&message=${message}`);
        });
        ...
        it('encodes a url with recipient, amount and memo', () => {
            ...
            expect(String(url)).toBe(`trezoa:${recipient}?amount=100&memo=${memo}`);
        });
    });
    ...
    describe('TransactionRequestURL', () => {
        it('encodes a URL', () => {
            ...
            expect(String(url)).toBe(`trezoa:${link}?label=${label}&message=${message}`);
        });
        ...
        it('encodes a URL with query parameters', () => {
            ...
            expect(String(url)).toBe(`trezoa:${encodeURIComponent(link)}?label=${label}&message=${message}`);
        });
    });
});
